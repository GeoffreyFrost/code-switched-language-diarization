...
class SoapiePreprocessor():
    ...
    def cosntruct_utt_audio_path_map(self):
        spkr_utt_paths = {}
        for (root,dirs,files) in os.walk(self.audio_dir, topdown=True):
            for file in files:
                if file.split('.')[-1]=='wav':
                    spkr = os.path.split(root)[-1]
                    utt_path = os.path.join(root, file)
    ...
    def generate_split_dataframes(self, save_pkl=False):
...
def construct_datadict(spkr_utt_paths, lang_tgts_dir):
    ...
    for spkr_utt in tqdm(spkr_utt_paths):
        tgts = load_utt_tgts(spkr_utt, lang_tgts_dir)
        if tgts.any() != False:
            datadict['audio_fpath'].append(spkr_utt_paths[spkr_utt])
            datadict['tgts'].append(tgts)
    return datadict