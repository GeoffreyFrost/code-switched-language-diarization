...
from baseline_models import BLSTM_E2E_LID
...
class lit_blmstm_e2e(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # copy pasta
        x_ = rnn_util.pack_padded_sequence(x, x_l.cpu(), batch_first=True)
        y_ = rnn_util.pack_padded_sequence(y, x_l.cpu(), batch_first=True).data.to(x.device)
        y_hat, embeddings = self.forward(x_)

        loss_DCL = loss_func_DCL(embeddings, y_)
        loss_CRE = loss_func_CRE(y_hat, y_)

        loss = 0.5 * loss_CRE + (1 - 0.5) * loss_DCL

        self.log('train/joint_loss', loss, sync_dist=True)
        return {'loss':loss, 'y_hat':y_hat.detach(), 'y':y.detach(), 'lengths':x_l.detach()}
